...
Transitions** or **SMART** surveys) or posterior weighting (e.g. as used
in **Rapid Assessment Method** or **RAM** and **Simple Spatial Sampling
Method** or **S3M** surveys).

## Details

The bootstrap technique is described in this
[article](https://en.wikipedia.org/wiki/Bootstrapping_(statistics)). The
BBW used in RAM and S3M is a modification to the *percentile bootstrap*
to include **blocking** and **weighting** to account for a complex
sample design.
...
for the sample design. A blocked weighted bootstrap (BBW) can be used:
...
- **Weighted** - RAM and S3M samples do not use population proportional
  ...
  to weight (i.e. by population) the selection probability of PSUs in
...
estimate consists of the **0.025th (95% LCL)**, **0.5th (point
estimate)**, and **0.975th (95% UCL)** quantiles of the distribution of
the statistic across all survey replicates.

Early versions of the `{bbw}` package did not resample observations
within PSUs following:
...
and used a large number (e.g. 3999) survey replicates. Current versions
of the `{bbw}` package resample observations within PSUs and use a
smaller number of survey replicates (e.g. `n = 400`). This is a more
...
([ORCID](https://orcid.org/0000-0002-4887-4415)) \[copyright holder\]
